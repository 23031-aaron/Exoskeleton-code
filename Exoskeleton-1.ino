...
uint8_t receiverMAC[] = {0xAA, 0xBB, 0xCC, 0xDD, 0xEE, 0xFF}; // put your receiver's MAC here
...
SensorLeg legA = { MPU_ADDR_A, 0, 0, 0, 0, "A" };
SensorLeg legB = { MPU_ADDR_B, 0, 0, 0, 0, "B" };
...
void loop() {
  ...
  Serial.print("A:"); Serial.print(outgoing.tiltA);
  Serial.print(" B:"); Serial.println(outgoing.tiltB);
  delay(50);
}
...
void calibrate(SensorLeg &leg) {
  ...
  Serial.print("Leg "); Serial.print(leg.name);
  Serial.print(" baseline: "); Serial.println(leg.baseline);
}

void readLeg(SensorLeg &leg, int &tiltOut, bool &risingOut) {
  int raw = readAccelX(leg.mpuAddr) - leg.baseline;

  if (abs(raw - leg.prevTilt) > SPIKE_LIMIT) {
  ...
  leg.smoothed = (EMA_ALPHA * raw) + ((1 - EMA_ALPHA) * leg.smoothed);
  int tilt = (int)leg.smoothed;
  int change = tilt - leg.prevTilt;

  bool candidate = (tilt > DEAD_ZONE) && (change > RISE_THRESHOLD);
  leg.riseCount = candidate ? leg.riseCount + 1 : 0;

  tiltOut = tilt;
  risingOut = (leg.riseCount >= DEBOUNCE_COUNT);
  leg.prevTilt = tilt;
}
